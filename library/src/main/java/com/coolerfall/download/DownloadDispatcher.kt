...
internal class DownloadDispatcher(
	...
	private fun updateState(request: DownloadRequest?, state: DownloadState) {
	...
	private fun updateStart(
		...
		/* if the request has failed before, don not deliver callback */
	...
	private fun updateFailure(
		...
		val leftRetryTime = request.retryTime.decrementAndGet()
		if (leftRetryTime >= 0) {
			try {
